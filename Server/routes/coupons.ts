...
// POST /api/coupons/child-profile
  ...
  try {
    ...
    if (!user_id || !child_name || !birth_date) {
      return res.status(400).json({
        message: "Thiếu user_id, child_name hoặc birth_date",
      });
    }
...
// GET /api/coupons/child-profiles/:userId
...
// GET /api/coupons/birthday/:userId
router.get("/birthday/:userId", async (req: Request, res: Response) => {
  try {
    ...
    for (const child of upcomingChildren) {
      ...
      const couponResult = await client.query(
        ...
        INSERT INTO birthday_coupons
          (coupon_id, user_id, child_id, code, discount_percent, expires_at)
        VALUES
          ($1, $2, $3, $4, $5, CURRENT_DATE + INTERVAL '14 days')
    ...
    return res.status(200).json({
      message: "Birthday coupons checked",
      data: coupons,
    });
  } catch (error) {
...
// GET /api/coupons/gift-suggestions/:userId
