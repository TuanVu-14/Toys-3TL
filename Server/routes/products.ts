...
import {
  ...
  getReviewSchema,
  productIDSchema,
} from "../validators/productsValidation";
...
async function calculateStarAverage(productID: string) {
  ...
  try {
    ...
    const totalStars = result.rows.reduce(
...
router.post("/product/create", async (req: Request, res: Response) => {
  ...
  try {
    ...
    await client.query(productImagesQuery, [
      imageID,
      productID,
      imgLink,
      imgAlt,
    ...
    await client.query(productParamsQuery, [
      productID,
      isSale,
      isNew,
      isDiscount,
...
async function review(productID: string) {
  try {
    const result = await client.query(
      `SELECT reviews.reviewid,reviews.userid,reviews.rating,reviews.title,reviews.comment,users.username,reviews.createdat,productparams.stars AS productstars FROM reviews INNER JOIN users ON users.userid = reviews.userid INNER JOIN productparams ON reviews.productid = productparams.productid WHERE reviews.productid = $1 ORDER BY reviews.createdat LIMIT 10`,
      [productID],
    );
    if (result.rows.length === 0) {
      return [0, []];
...
async function getImages(productID: string) {
   try {
        const result = await client.query(
            `SELECT imageid,imglink,imgalt FROM productimages 
             WHERE productid = $1 AND isprimary = false 
...
router.get(
  ...
  async (req: Request, res: Response) => {
    ...
    if (result.isEmpty()) {
      ...
      brands.manufacturer AS manufacturer_info,
      brands.safety_certificates AS certification_details,

      -- Collection information
      COALESCE(
        STRING_AGG(DISTINCT collections.name, ', '),
        ''
      ) AS collection_names
   ...
   INNER JOIN productimages
      ON productimages.productid = products.productid
   INNER JOIN sellers
      ON sellers.seller_id = products.seller_id
   INNER JOIN categories
      ON products.categoryid = categories.categoryid

   -- Brand
   LEFT JOIN brands
      ON products.brand = brands.name
   -- Collections
   LEFT JOIN collection_products
      ON products.productid = collection_products.productid
   LEFT JOIN collections
      ON collection_products.collection_id = collections.collection_id
...
router.patch(
  ...
  async (req: Request, res: Response) => {
    const result = validationResult(req);
    if (result.isEmpty()) {
      const { reviewID, userID, productID, rating, title, comment } =
        matchedData(req);
