...
function CollectionsContent() {
  ...
  const filteredCollections = useMemo(() => {
    ...
    return collections.filter((c) => [c.name, c.slug, c.description].some((v) => String(v || "").toLowerCase().includes(kw)));
  }, [collections, search]);
...
<div className="flex gap-2 flex-wrap">
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Tìm tên, slug..." className="rounded-2xl border border-slate-200 px-4 py-2 text-sm outline-none focus:border-rose-400" />
    ...
    <section className="rounded-3xl border border-slate-100 bg-white p-6 shadow-sm">
      ...
      <div className="mt-4 space-y-3">
        ...
        {filteredCollections.map((collection) => <div key={collection.collectionid} className="flex items-center justify-between rounded-2xl border border-slate-100 p-4">
