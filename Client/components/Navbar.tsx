        ...
        <div className="flex w-[80%] justify-between">
          <div className="flex gap-2">
            {socialMedia.map((each, index) => (
              <button key={index} className="text-[16px] text-silver bg-gray-200 w-[25px] rounded-md hover:bg-blueIn hover:text-white">
      ...
      <div className="w-full h-auto flex flex-col justify-between items-center bg-flower-50">
        <div className="flex justify-evenly items-center w-full flex-col sm:flex-row gap-2 sm:gap-0">
          <div className="w-[80%] flex justify-between items-center flex-col sm:flex-row sm:gap-0">
            <Link className="mr-2.5 text-[26px] text-flower-150 font-bold" href="/">
      ...
      >
        <div className="flex">
          {navBtns.map((btn, index) => (
            <div key={index} onMouseEnter={() => openDropdown(index)} className="relative items-center">
              <button
                ...
                <span className="relative inline-block px-3 py-1">
                  ...
                  {btn.name === 'Sale' && (
                    <svg viewBox="0 0 120 40" className="absolute -top-2 -left-2 w-[130%] h-[160%] pointer-events-none">
                      <ellipse cx="60" cy="20" rx="55" ry="16" stroke="#E8989A" strokeWidth="2" fill="none" />
                    </svg>
                  )}
                </span>
              </button>
              ...
              {selectIndex === index && isDropdownVisible && btn.name === 'Categories' && (
                <div onMouseEnter={() => openDropdown(index)} onMouseLeave={closeDropdown}>
                  <Category />
                </div>
              )}

              {selectIndex === index && btn.isExtendable && isDropdownVisible && btn.name !== 'Categories' && (
                <div onMouseEnter={() => openDropdown(index)} onMouseLeave={closeDropdown}>
